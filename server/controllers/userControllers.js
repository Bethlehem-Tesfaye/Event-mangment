...
export const setProfile = async (req, res, next) => {
  ...
  try {
    const query= 'UPDATE profiles SET first_name=$1, last_name=$2, phone=$3, address=$4, country=$5, city=$6 WHERE user_id=$7 RETURNING *';

    const result = await conn.query(query, [first_name, last_name, phone, address, country, city, userId]);

    return res.status(200).json({ success: true, data:result.rows[0], message: "Profile updated" });
